...
def parse_args(args):
    ...
    parser.add_argument('--epochs', help='Number of epochs.', default=4)
...
def main(args=None, configs=None):
    ...
    s2s.model.summary()
    plot_model(s2s.model, to_file=snapshot_path+'.png', show_shapes=True, show_layer_names=True)
    ...
    if args.model == 's2srnn':
        s2s.model.fit([Xtrain, Ytrain], None,  batch_size=args.batch_size, epochs=args.epochs,
                      validation_data=([Xvalid, Yvalid], None),
                      callbacks=[model_saver, csv_logger])
    elif args.model == 'transformer':
        s2s.model.fit([Xtrain, Ytrain], None, batch_size=args.batch_size, epochs=args.epochs,
                      validation_data=([Xvalid, Yvalid], None), shuffle=True,
